"""Red Hat Supplementary Style Guide mapping utility.

Central system for managing Red Hat SSG rule citations.
Loads rule mappings from red_hat_style_mapping.yaml and provides
lookup, citation formatting, confidence adjustment, and
verification status functions.

Usage:
    from style_guides.red_hat.red_hat_style_mapping import get_rule_mapping, format_citation

    mapping = get_rule_mapping('inclusive_language')
    citation = format_citation('product_names')
"""
...
# Constants
RED_HAT_SSG_DEFAULT_CITATION = "Red Hat Supplementary Style Guide"
...
class RedHatStyleMapping:
    """Singleton class for Red Hat SSG mapping management.

    Loads the red_hat_style_mapping.yaml file once and provides
    methods to look up rule mappings, format citations, compute
    confidence adjustments, and check verification status.
    """

    _instance: Optional['RedHatStyleMapping'] = None
    _mapping: Optional[Dict[str, Any]] = None
    _loaded: bool = False

    def __new__(cls) -> 'RedHatStyleMapping':
    ...
    def _load_mapping(self) -> None:
        """Load the Red Hat SSG mapping YAML file."""
        current_dir = Path(__file__).parent
        mapping_file = current_dir / 'red_hat_style_mapping.yaml'

        if not mapping_file.exists():
            logger.warning(
                "Red Hat SSG mapping file not found at %s, using defaults",
                mapping_file,
            )
            self._mapping = self._get_default_mapping()
            RedHatStyleMapping._loaded = True
            return
        ...
        try:
            ...
            RedHatStyleMapping._loaded = True
            logger.info("Red Hat SSG mapping loaded successfully")
        except yaml.YAMLError as exc:
            logger.error("YAML parse error loading Red Hat SSG mapping: %s", exc)
            self._mapping = self._get_default_mapping()
            RedHatStyleMapping._loaded = True
        except FileNotFoundError:
            logger.error("Red Hat SSG mapping file disappeared during read")
            self._mapping = self._get_default_mapping()
            RedHatStyleMapping._loaded = True
    ...
    def get_rule_mapping(
        ...
        Args:
            rule_id: Rule identifier (e.g., 'inclusive_language').
            category: Optional category hint (e.g., 'conscious_language').
    ...
    def format_citation(
        ...
        Returns:
            ...
            ``"Red Hat Supplementary Style Guide (Section: conscious-language)"``.
        """
        mapping = self.get_rule_mapping(rule_id, category)
        if not mapping:
            return RED_HAT_SSG_DEFAULT_CITATION

        red_hat_ssg = mapping.get('red_hat_ssg', {})
        return self._build_citation_string(red_hat_ssg, include_verification)

    def _build_citation_string(
        self,
        red_hat_ssg: Dict[str, Any],
        include_verification: bool,
    ) -> str:
        """Build a formatted citation string from mapping data.

        Args:
            red_hat_ssg: The red_hat_ssg sub-dict from the mapping.
            include_verification: Whether to append verification indicator.

        Returns:
            Formatted citation string.
        """
        verification_status = red_hat_ssg.get('verification_status', 'unverified')
        section = red_hat_ssg.get('section', '')
        topic = red_hat_ssg.get('topic', 'Style Guidelines')

        if section:
            citation = f"{RED_HAT_SSG_DEFAULT_CITATION} (Section: {section})"
        else:
            citation = f"{RED_HAT_SSG_DEFAULT_CITATION}: {topic}"
    ...
    def get_excerpt(
        ...
        red_hat_ssg = mapping.get('red_hat_ssg', {})
        return {
            'guide_name': RED_HAT_SSG_DEFAULT_CITATION,
            'topic': red_hat_ssg.get('topic', ''),
            'pages': [],
            'excerpt': red_hat_ssg.get('excerpt', red_hat_ssg.get('guidance', '')),
            'verified': red_hat_ssg.get('verification_status') == 'verified',
        }
    ...
    def get_verification_status(
        ...
        red_hat_ssg = mapping.get('red_hat_ssg', {})
        return {
            'status': red_hat_ssg.get('verification_status', 'unverified'),
            'verified': red_hat_ssg.get('verification_status') == 'verified',
            'section': red_hat_ssg.get('section'),
            'topic': red_hat_ssg.get('topic'),
        }
        ...
        for category in _CATEGORIES:
            ...
            for rule_id, rule_data in cat_data.items():
                if not isinstance(rule_data, dict):
                    continue
                red_hat_ssg = rule_data.get('red_hat_ssg', {})
                if red_hat_ssg.get('verification_status') != 'verified':
                    unverified.append({
                        'rule_id': rule_id,
                        'category': category,
...
_mapping_instance: Optional[RedHatStyleMapping] = None


def _get_mapping_instance() -> RedHatStyleMapping:
    """Get the singleton instance of RedHatStyleMapping.

    Returns:
        The shared RedHatStyleMapping singleton.
    """
    global _mapping_instance  # noqa: PLW0603
    if _mapping_instance is None:
        _mapping_instance = RedHatStyleMapping()

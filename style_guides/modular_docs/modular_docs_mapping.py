"""Modular Documentation Reference Guide mapping utility.

Central system for managing modular documentation rule citations.
Loads rule mappings from modular_docs_mapping.yaml and provides
lookup, citation formatting, confidence adjustment, and
verification status functions.

Usage:
    from style_guides.modular_docs.modular_docs_mapping import get_rule_mapping

    mapping = get_rule_mapping('procedure_module')
...
# Constants
MODULAR_DOCS_DEFAULT_CITATION = "Modular Documentation Reference Guide"
...
class ModularDocsMapping:
    """Singleton class for modular documentation mapping management.

    Loads the modular_docs_mapping.yaml file once and provides
    methods to look up rule mappings, format citations, compute
    confidence adjustments, and check verification status.
    """

    _instance: Optional['ModularDocsMapping'] = None
    _mapping: Optional[Dict[str, Any]] = None
    _loaded: bool = False

    def __new__(cls) -> 'ModularDocsMapping':
    ...
    def _load_mapping(self) -> None:
        """Load the modular docs mapping YAML file."""
        current_dir = Path(__file__).parent
        mapping_file = current_dir / 'modular_docs_mapping.yaml'

        if not mapping_file.exists():
            logger.warning(
                "Modular docs mapping file not found at %s, using defaults",
                mapping_file,
            )
            self._mapping = self._get_default_mapping()
            ModularDocsMapping._loaded = True
            return
        ...
        try:
            ...
            ModularDocsMapping._loaded = True
            logger.info("Modular docs mapping loaded successfully")
        except yaml.YAMLError as exc:
            logger.error("YAML parse error loading modular docs mapping: %s", exc)
            self._mapping = self._get_default_mapping()
            ModularDocsMapping._loaded = True
        except FileNotFoundError:
            logger.error("Modular docs mapping file disappeared during read")
            self._mapping = self._get_default_mapping()
            ModularDocsMapping._loaded = True
    ...
    def get_rule_mapping(
        ...
        Args:
            rule_id: Rule identifier (e.g., 'procedure_module').
            category: Optional category hint (e.g., 'module_types').
    ...
    def format_citation(
        ...
        if not mapping:
            return MODULAR_DOCS_DEFAULT_CITATION

        modular_docs = mapping.get('modular_docs', {})
        return self._build_citation_string(modular_docs, include_verification)

    def _build_citation_string(
        self,
        modular_docs: Dict[str, Any],
        include_verification: bool,
    ) -> str:
        """Build a formatted citation string from mapping data.

        Args:
            modular_docs: The modular_docs sub-dict from the mapping.
            include_verification: Whether to append verification indicator.

        Returns:
            Formatted citation string.
        """
        verification_status = modular_docs.get('verification_status', 'unverified')
        section = modular_docs.get('section', '')
        topic = modular_docs.get('topic', 'Modular Documentation')

        if section:
            citation = f"{MODULAR_DOCS_DEFAULT_CITATION} (Section: {section})"
        else:
            citation = f"{MODULAR_DOCS_DEFAULT_CITATION}: {topic}"
    ...
    def get_excerpt(
        ...
        modular_docs = mapping.get('modular_docs', {})
        return {
            'guide_name': MODULAR_DOCS_DEFAULT_CITATION,
            'topic': modular_docs.get('topic', ''),
            'pages': [],
            'excerpt': modular_docs.get('excerpt', modular_docs.get('guidance', '')),
            'verified': modular_docs.get('verification_status') == 'verified',
        }
    ...
    def get_verification_status(
        ...
        modular_docs = mapping.get('modular_docs', {})
        return {
            'status': modular_docs.get('verification_status', 'unverified'),
            'verified': modular_docs.get('verification_status') == 'verified',
            'section': modular_docs.get('section'),
            'topic': modular_docs.get('topic'),
        }
        ...
        for category in _CATEGORIES:
            ...
            for rule_id, rule_data in cat_data.items():
                if not isinstance(rule_data, dict):
                    continue
                modular_docs = rule_data.get('modular_docs', {})
                if modular_docs.get('verification_status') != 'verified':
                    unverified.append({
                        'rule_id': rule_id,
                        'category': category,
...
_mapping_instance: Optional[ModularDocsMapping] = None


def _get_mapping_instance() -> ModularDocsMapping:
    """Get the singleton instance of ModularDocsMapping.

    Returns:
        The shared ModularDocsMapping singleton.
    """
    global _mapping_instance  # noqa: PLW0603
    if _mapping_instance is None:
        _mapping_instance = ModularDocsMapping()

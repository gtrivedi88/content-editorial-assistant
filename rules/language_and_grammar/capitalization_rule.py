...
class CapitalizationRule(BaseLanguageRule):
    ...
    def _is_part_of_product_name(self, token, doc) -> bool:
        ...
        # Check 2 tokens before
        for offset in range(1, min(3, token.i + 1)):
            prev = doc[token.i - offset]
            if prev.text and prev.text[0].isupper() and prev.text.lower() not in ('the', 'a', 'an', 'in', 'on', 'for', 'of', 'to', 'with'):
                return True
            if not prev.text[0].isupper():
                break

        # Check 2 tokens after
        for offset in range(1, min(3, len(doc) - token.i)):
            nxt = doc[token.i + offset]
            if nxt.text and nxt.text[0].isupper():
                return True
            if not nxt.text[0].isupper():
                break

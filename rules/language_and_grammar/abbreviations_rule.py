...
class AbbreviationsRule(BaseLanguageRule):
    ...
    def _should_skip_abbreviation(self, token, sentence, context) -> bool:
        ...
        # AsciiDoc metadata/structural lines
        stripped_sent = sentence.text.strip()
        if token.text.isupper() and (
            stripped_sent.startswith(':')        # attribute: :key: VALUE
            or stripped_sent.startswith('[')     # block attributes: [role=VALUE]
            or stripped_sent.startswith('.')     # block title: .Title
            or stripped_sent.startswith('=')     # heading: = TITLE
            or stripped_sent.startswith('//')    # comment: // COMMENT
            or stripped_sent.startswith('ifdef') # conditional: ifdef::ATTR[]
            or stripped_sent.startswith('ifndef')
            or stripped_sent.startswith('include::')
        ):
            return True

...
class DefinitionsRule(BaseLanguageRule):
    ...
    def analyze(self, text: str, sentences: List[str], nlp=None,
        ...
        # Include document-wide definitions from orchestrator
        doc_acronyms = context.get("acronym_context", {})
        if doc_acronyms:
            defined.update(doc_acronyms.keys())

        code_ranges = context.get("inline_code_ranges", [])

        # Find all used acronyms
        seen_acronyms: set = set()
        doc = spacy_doc if spacy_doc is not None else None
        if doc is None and nlp:
            doc = nlp(text)

        if doc is not None:
            for i, sent in enumerate(doc.sents):
                self._check_spacy_sent(
                    sent, i, text, context, defined, code_ranges,
                    seen_acronyms, errors,
                )
        else:
            for i, sentence in enumerate(sentences):
                self._check_plain_sent(
                    sentence, i, text, context, defined, code_ranges,
                    seen_acronyms, errors,
                )

        return errors

    def _check_spacy_sent(self, sent, idx, text, context, defined,
                          code_ranges, seen_acronyms, errors):
        """Check a SpaCy sentence span for undefined acronyms."""
        for match in _ACRONYM_USE.finditer(sent.text):
            ...
            error = self._create_error(
                sentence=sent.text, sentence_index=idx,
                message=(
                ...
                flagged_text=acronym, span=(start, end),
            )
            if error:
                errors.append(error)

    def _check_plain_sent(self, sentence, idx, text, context, defined,
                          code_ranges, seen_acronyms, errors):
        """Check a plain sentence string for undefined acronyms."""

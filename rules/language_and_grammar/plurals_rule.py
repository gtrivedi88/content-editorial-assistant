...
class PluralsRule(BaseLanguageRule):
    ...
    def analyze(self, text: str, sentences: List[str], nlp=None,
        ...
        errors.extend(self._check_plural_adjectives(doc, text, context, code_ranges))
    ...
    def _check_plural_adjectives(self, doc, text: str, context,
                                 code_ranges: list = None) -> List[Dict[str, Any]]:
        ...
        for i, sent in enumerate(doc.sents):
            for token in sent:
                if self._is_plural_adjective(token, doc, code_ranges):
                    error = self._create_error(
                        sentence=sent.text,
                        sentence_index=i,
                        message=(
    ...
    def _is_plural_adjective(self, token, doc, code_ranges: list = None) -> bool:
        ...
        if self._fails_surface_guards(token, doc, code_ranges):
            return False
    ...
    def _fails_surface_guards(self, token, doc, code_ranges: list = None) -> bool:
        ...
        # Skip tokens inside inline code

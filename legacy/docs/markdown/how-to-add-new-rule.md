...
Add your rule to the appropriate package's `_init_.py` file to enable automatic discovery.

#### For Language & Grammar Rules

Edit `rules/language_and_grammar/_init_.py`:
[code] 
    """
...
Edit `rules/punctuation/_init_.py`:
[code] 
    """
...
Edit `rules/structure_and_format/_init_.py`:
[code] 
    """
...
`rules/category/_init_.py` | Package imports | ✅ Required  
...
`rules/_init_.py` | Registry modifications | Advanced registry changes  
...
**Solutions** :
  ...
  4. Check `_init_.py`: Ensure rule is imported
...
### File: Update `rules/language_and_grammar/_init_.py`
[code] 
    """
    Language and Grammar Rules Package
    """

 ...
 * Replaces ProgressDisplay. Shows all 11 IBM Style Guide groups sweeping
 * from pending → active → done based on analysis progress percentage.
 */
...
export class CheckingIndicator {
    constructor(containerEl, store) {
        ...
        store.subscribe('progressPercent', (pct) => {
    ...
    _startSweep() {
        ...
        const next = () => {
            ...
            // Don't mark the last one done here — _hide() removes the whole indicator
        };
    ...
    _updateProgress(percent) {
        // Progress updates from WebSocket are handled by the sweep animation.
        // The sweep runs independently for smooth UX.
    }
}

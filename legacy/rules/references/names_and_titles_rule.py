...
_TITLES = {
    ...
    'engineer', 'developer', 'architect', 'analyst',
...
class NamesAndTitlesRule(BaseReferencesRule):
    ...
    def analyze(self, text: str, sentences: List[str], nlp=None,
        ...
        for idx, sent in enumerate(doc.sents):
            code_ranges = _inline_code_ranges(sent.text)
    ...
    def _check_standalone_title_caps(self, sent, idx, text, context,
        ...
        for i, token in enumerate(tokens):
                ...
                if i + 1 < len(tokens):
                    ...
                    if two_word in _TITLES and tokens[i + 1].text[0].isupper():
                        if self._is_standalone(tokens, i, sent, doc):
                            if not _in_ranges(token.idx - sent.start_char, code_ranges):
            ...
            if _in_ranges(token.idx - sent.start_char, code_ranges):
                continue

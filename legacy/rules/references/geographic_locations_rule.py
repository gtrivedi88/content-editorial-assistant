...
class GeographicLocationsRule(BaseReferencesRule):
    ...
    def analyze(self, text: str, sentences: List[str], nlp=None,
        ...
        for idx, sentence in enumerate(sentences):
            ...
            self._check_outdated_terms(sentence, idx, text, context, code_ranges, errors)
            self._check_misused_terms(sentence, idx, text, context, code_ranges, errors)
        return errors
    ...
    def _check_misused_terms(self, sentence, idx, text, context,
        ...
        for term, info in _MISUSED_TERMS.items():
            ...
            for wrong_ctx in wrong_contexts:
                # Match patterns like "state" near a wrong context word
                pattern = re.compile(
                    r'\b' + re.escape(term) + r'\b.*?\b' + re.escape(wrong_ctx) + r'\b'
                    r'|'
                    r'\b' + re.escape(wrong_ctx) + r'\b.*?\b' + re.escape(term) + r'\b',
                    re.IGNORECASE,
                )
                for match in pattern.finditer(sentence):
                    if _in_ranges(match.start(), code_ranges):

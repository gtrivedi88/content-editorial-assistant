...
class CrossReferenceRule(BaseRule):
    ...
    def __init__(self):
        ...
        # Deprecated anchor prefixes per May 2023 update
    ...
    def analyze(self, text: str, sentences: List[str] = None, nlp=None, context: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            errors.extend(self._validate_deprecated_anchor_prefixes(anchors, text, context))
            errors.extend(self._check_xref_deprecated_prefixes(xrefs, text, context))
    ...
    def _validate_deprecated_anchor_prefixes(self, anchors: Set[str], text: str, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Validate that anchor IDs don't use deprecated module prefixes.
        
        Per Red Hat May 2023: Module prefixes (con_, proc_, ref_) were removed from anchors.
        """
        errors = []
        
        for anchor in anchors:
            for prefix in self.deprecated_anchor_prefixes:
                if anchor.lower().startswith(prefix):
                    # Find line number for this anchor
                    anchor_match = re.search(rf'\[\[{re.escape(anchor)}\]\]', text)
                    line_number = text[:anchor_match.start()].count('\n') + 1 if anchor_match else 0
                    
                    suggested_anchor = anchor[len(prefix):]
                    
                    errors.append(self._create_error(
                        anchor,  # sentence
                        line_number,  # sentence_index
                        f"Anchor uses deprecated '{prefix}' prefix per Red Hat May 2023",  # message
                        [
                            f"Remove prefix: [[{suggested_anchor}]] instead of [[{anchor}]]",
                        ...
                        context=context,
                        confidence=0.8,
                        error_type="deprecated_anchor_prefix",
                        flagged_text=anchor
                    ))
                    break
    ...
    def _check_xref_deprecated_prefixes(self, xrefs: List[Dict[str, Any]], text: str, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Check xref targets for deprecated anchor prefixes.
        
        Per Red Hat May 2023: xref targets should not reference anchors with
        con_, proc_, ref_ prefixes.
        """
        errors = []
        ...
        for xref in xrefs:
            ...
            # Extract anchor from target (after # if present, or whole target if no file path)
            if '#' in target:
                anchor = target.split('#')[1]
            elif not any(target.endswith(ext) for ext in self.valid_file_extensions):
                anchor = target
            else:
                continue
            ...
            for prefix in self.deprecated_anchor_prefixes:
                if anchor.lower().startswith(prefix):
                    ...
                    errors.append(self._create_error(
                        target,  # sentence
                        xref['line_number'],  # sentence_index
                        f"xref target uses deprecated '{prefix}' anchor prefix",  # message
                        [
                            f"Update target to reference: {target.replace(anchor, suggested_anchor)}",
                            "Per Red Hat May 2023: Module prefixes removed from anchors"
                        ],  # suggestions
                        severity="low",
                        text=text,
                        context=context,
                        confidence=0.6,
                        error_type="xref_deprecated_prefix",
                        flagged_text=target
                    ))
                    break

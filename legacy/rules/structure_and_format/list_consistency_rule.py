...
_SENTENCE_PATTERN = re.compile(
    r'^[A-Z].*\s(?:is|are|was|were|has|have|had|do|does|did|can|could|'
    r'will|would|shall|should|may|might|must|need|use|click|select|'
    r'open|close|create|delete|run|start|stop|set|get|make|take|give|'
    r'check|verify|ensure|see|refer|go|type|enter|press|drag|drop|'
    r'install|configure|update|remove|add|copy|move|save|load|'
    r'provides?|grants?|limits?|allows?|enables?|contains?|includes?|'
    r'specif(?:y|ies)|describ(?:e|es)|indicat(?:e|es)|defin(?:e|es))\b',
    re.IGNORECASE
)

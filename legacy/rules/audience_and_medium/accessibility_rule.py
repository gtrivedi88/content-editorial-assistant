...
_KNOWN_ACRONYMS = {
    ...
    'IBM', 'AWS', 'GCP', 'CLI', 'GUI', 'SDK', 'JVM', 'JDK', 'IDE',
    'CPU', 'GPU', 'RAM', 'SSD', 'HDD', 'NFS', 'LDAP', 'RBAC', 'YAML',
    'CI', 'CD', 'VM', 'OS', 'IP', 'UI', 'UX', 'ID', 'OK', 'PDF',
    ...
    def _check_all_caps(self, sent, sent_index, text, context, errors):
        ...
        for token in sent:
            ...
            if token.text in _KNOWN_ACRONYMS:
                continue
            # Skip if short enough to be an acronym (6 chars or less)
            if len(token.text) <= 6:
                continue

...
class CommandsRule(BaseTechnicalRule):
    ...
    def analyze(self, text: str, sentences: List[str], nlp=None,
        ...
        for idx, sentence in enumerate(sentences):
            self._check_allcaps_command_as_verb(sentence, idx, text, context, errors)
        return errors

    def _check_allcaps_command_as_verb(self, sentence, idx, text, context, errors):
...
def _in_backticks(text: str, pos: int) -> bool:
    """Return True if position is inside backtick-delimited code."""
    before = text[:pos]

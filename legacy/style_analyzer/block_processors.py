...
class BlockProcessor:
    ...
    def _flatten_recursively(self, block: Any):
        ...
        # For container types, process children only — don't add the container itself.
        # These blocks have empty raw_content; their children hold the actual content.
        container_types = [
            'preamble', 'table_row',
            'admonition', 'example', 'sidebar', 'quote', 'verse', 'open',
        ]
        if block_type_value in container_types:
            for child in block.children:
                self._flatten_recursively(child)
            return
        ...
        # List containers (olist, ulist, dlist): recurse into children (list_items)
        # so each item gets its own start_pos and analysis.
        if block_type_value in ['olist', 'ulist', 'dlist']:
            for child in block.children:
                self._flatten_recursively(child)
            return

        # List items may have empty raw_content with children holding the text
        # (e.g., a paragraph child). Recurse into children if available,
        # otherwise skip — empty list items have no analyzable content.
        if block_type_value in ['list_item', 'description_list_item']:
        ...
        # For all other displayable block types (paragraphs, headings, etc.),
        # add them directly to the flat list.
        self.flat_blocks.append(block)

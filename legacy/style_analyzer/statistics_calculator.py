...
class StatisticsCalculator:
    ...
    def calculate_comprehensive_statistics(self, text: str, sentences: List[str], paragraphs: List[str]) -> StatisticsDict:
        ...
        try:
            ...
            # Add word analysis (limit text size for performance)
            word_stats = self._calculate_word_statistics_safe(text)
    ...
    def _calculate_word_statistics(self, text: str) -> Dict[str, Any]:
        ...
        return self._calculate_word_statistics_safe(text)

    def _calculate_word_statistics_safe(self, text: str, max_words: int = 50000) -> Dict[str, Any]:
        """Calculate word-level statistics with performance safeguards."""
        stats = {
            'avg_word_length': 0.0,
        ...
        try:
            ...
            # Performance safeguard: for very large documents, sample words
            if len(words) > max_words:
                logger.warning(f"Document has {len(words)} words, sampling {max_words} for word statistics")
            ...
            # Word frequency analysis (limit to prevent memory issues)
    ...
    def _calculate_readability_statistics(self, text: str) -> Dict[str, Any]:
        """Calculate readability statistics using textstat with timeout protection."""
        stats = {
        ...
        # For very large documents, truncate to prevent timeout
        max_chars = 100000  # ~20K words
        if len(text) > max_chars:
            logger.warning(f"Document has {len(text)} chars, truncating to {max_chars} for readability metrics")
            text = text[:max_chars]
    ...
    def calculate_engagement_metrics(self, text: str, statistics: Dict[str, Any]) -> Dict[str, Any]:
        """
        Calculate engagement and tone metrics.
        
        This delegates to MetricsCalculator.calculate_engagement_metrics() which is the
        AUTHORITATIVE source for engagement scoring.
        ...
        try:
            ...
            return MetricsCalculator.calculate_engagement_metrics(text, statistics)
        except ImportError:
            logger.warning("MetricsCalculator not available for engagement metrics")
            return {
                'engagement_score': 0,

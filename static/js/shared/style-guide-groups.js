...
const TYPE_TO_GROUP = new Map([
    ...
    // LLM-sourced issues (mapped by LLM category)
    ['llm_style', 'audience_and_medium'],
    ['llm_grammar', 'language_and_grammar'],
    ['llm_punctuation', 'punctuation'],
    ['llm_structure', 'structure_and_format'],
    ['llm_audience', 'audience_and_medium'],
]);
...
export function formatRuleType(type) {
    ...
    // LLM-sourced rule names: display the category without "llm_" prefix
    if (type.startsWith('llm_')) {
        const category = type.slice(4);

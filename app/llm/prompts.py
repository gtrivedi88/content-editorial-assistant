...
def build_global_prompt(
    ...
    system_prompt = (
        ...
        "## IN SCOPE — CHECK these document-level issues\n"
        "- Tone consistency — voice shifts across the document\n"
        "- Flow — abrupt topic changes, missing transitions\n"
        "- Minimalism — unnecessary background or excessive detail\n"
        "- Wordiness — sections that could be more concise\n"
        "- Audience — inconsistent technical level\n"
        "- Document structure — verify modular docs patterns "
        ...
        "format)\n"
        "- Accessibility — instructions should not rely solely on visual "
        "cues (color, position, shape)\n"
        ...
        "- Future tense — flag uses of 'will' where simple present tense "
        "is correct (e.g., 'will fail' → 'fails', 'will be used' → "
        "'is used')\n"
        "- Single-step numbered procedures — if a procedure has only one "
        "step and it uses a numbered list (1.), flag it; single-step "
        "procedures should use an unnumbered bullet\n"
        "- Procedural content inside admonitions — procedures or steps "
